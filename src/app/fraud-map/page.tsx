...
export default function FraudMapPage() {
  ...
  const geojsonStats = useMemo(() => ({
    ...
    features: reports.map(r => ({
      ...
      properties: { id: r._id, city: r.city, severity: r.severity }
    }))
  }), [reports]);
  ...
  return (
    <div className="h-screen w-screen bg-[#020202] text-[#F2F2F2] overflow-hidden font-mono relative">
      ...
      <header className="absolute top-0 left-0 right-0 h-16 bg-[#040404]/80 backdrop-blur-md border-b border-[#0E0E0E] z-50 px-6 flex items-center justify-between">
         <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
                <h1 className="text-sm font-bebas tracking-[2px] text-white">FRAUD_INTELLIGENCE_SYSTEM</h1>
                <span className="text-[8px] text-[#39FF14] tracking-[1px] animate-pulse">ACTIVE_SURVEILLANCE_NODE_V4.2</span>
            </div>
         </div>
      ...
      <aside className="absolute left-6 top-24 bottom-6 w-80 bg-[#040404]/80 backdrop-blur-xl border border-[#0E0E0E] z-40 p-6 flex flex-col gap-6">
      ...
      <aside className="absolute right-6 top-24 bottom-6 w-64 bg-[#040404]/80 backdrop-blur-xl border border-[#0E0E0E] z-40 p-6 flex flex-col gap-8">
          <div className="flex flex-col gap-1">
      ...
      <div className="absolute inset-0 z-0">
        ...
        >
          ...
          {geojsonStats.features.length > 0 && (
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            <Source id="fraud-reports" type="geojson" data={geojsonStats as any}>
              <Layer
                id="fraud-heat"
                type="heatmap"
                paint={{
          ...
          {reports.map(report => (
            ...
            >
              <div className="relative group cursor-pointer">
                <div className={`w-3 h-3 border-2 rotate-45 animate-pulse ${
                    report.severity === 'CRITICAL' ? 'border-[#FF006E]' : 'border-[#39FF14]'
                }`} />
                <div className="absolute top-1/2 left-full ml-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-[#060606] border border-[#0E0E0E] p-2 z-50">
                    <p className="text-[10px] font-bold text-white">{report.city}</p>
                    <p className="text-[8px] text-[#FF006E]">{report.reason}</p>
                </div>
              </div>
            </Marker>
          ))}
      ...
      {/* SCANLINE EFFECTS */}
      <div className="absolute inset-0 pointer-events-none z-50 pointer-events-none opacity-[0.03] bg-[url('/noise.png')] mix-blend-overlay" />
    </div>
  );
}
